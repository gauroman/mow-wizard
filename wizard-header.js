 ...
 * @polymer
 */
class WizardHeader extends PolymerElement {
    static get template() {
        return html`
            <style>
            :host {
    ...
    _handleItemTap(e) {
      this.dispatchEvent(new CustomEvent('item-tap', { bubbles: true, composed: true, detail: {model: e.model, target:e.target} }));
    }   
}
window.customElements.define('wizard-header', WizardHeader);